...
plt.arrow(-6, -1, arrow_length * math.cos(arrow_angle), arrow_length * math.sin(arrow_angle), head_width=0.2, head_length=0.3, fc='purple', ec='purple')

# 矢印に対するラベルを追加（TeX表記）
plt.text(-6, -0.5,   r'$U_\infty$', fontsize=18, color='purple')
...
def plot_angle_mark(radius, angle_deg, label):
    ...
    if (angle_deg == 0):
        plt.text(end_x * 1.2, end_y * 1.1, label, fontsize=10, ha='center', va='center')
    else : 
        plt.text(end_x * 1.1, end_y * 1.1, label, fontsize=10, ha='center', va='center')
# 角度の目盛りを45度ごとに表示
for angle in range(0, 360, 45):
    plot_angle_mark(6, angle, f'{angle}°')
